...
class NRUCell(nn.Module):
    ...
    def __init__(self, input_size, hidden_size, memory_size=1152, k=2, use_relu=False, layer_norm=True):
        ...
        self.hm2v_alpha = nn.Linear(self.memory_size + input_size, 2 * sqrt_memk)
        self.hm2v_beta = nn.Linear(self.memory_size + input_size, 2 * sqrt_memk)
        self.hm2alpha = nn.Linear(self.memory_size + input_size, self.k)
        self.hm2beta = nn.Linear(self.memory_size + input_size, self.k)

        self.hmi2h = nn.Linear(self.memory_size + hidden_size + self.input_size, self.hidden_size//2)
        #self.hmi2h = nn.Linear(self.memory_size + self.input_size, hidden_size)
        self.h2h = nn.Linear(self.hidden_size//2, self.hidden_size)
    ...
    def forward(self, input, last_hidden):
        ...
        h = self._opt_layernorm(self.h2h(F.relu(self.hmi2h(c_input))))

        # Flat memory equations
        effective_h = torch.cat((input, last_hidden_memory), 1)
        alpha = self._opt_relu(self.hm2alpha(effective_h))
        beta = self._opt_relu(self.hm2beta(effective_h))

        u_alpha = self.hm2v_alpha(effective_h).chunk(2,dim=1)
        ...
        u_beta = self.hm2v_beta(effective_h).chunk(2, dim=1)
        ...
        hidden_memory = last_hidden_memory*torch.mean(add_memory - forget_memory, dim=1)
        hidden_h = h
...
class NRU(nn.Module):
    ...
    def __init__(self, input_size, hidden_size, num_layers=1, 
                 layer_norm=True, use_relu=False, memory_size=1225, k=4):
    ...
    def reset_hidden(self, batch_size):
        ...
        #h_n construction
        h_n = h[0][0].unsqueeze(0)
        m_n = h[0][1].unsqueeze(0)
        return (h_n, m_n)
